...
static void preProc(Live_graph lg)
{
    ...
    AdjMatrix = (char **)malloc(n * sizeof(*AdjMatrix));
    for (i = 0; i < n; i++)
    {
        AdjMatrix[i] = (char *)malloc(n * sizeof(char));
        memset(AdjMatrix[i], 0, n);
    }
    Alias = (G_node)malloc(n * sizeof(*Alias));
    memset(Alias, 0, n * sizeof(*Alias));
    Degree = (int *)malloc(n * sizeof(*Degree));
    memset(Degree, 0, n * sizeof(*Degree));
    NodeMoves = (list_t *)malloc(n * sizeof(*NodeMoves));
    memset(NodeMoves, 0, n * sizeof(*NodeMoves));
    Stack = (G_node *)malloc(n * sizeof(*Stack));
    sp = 0;
